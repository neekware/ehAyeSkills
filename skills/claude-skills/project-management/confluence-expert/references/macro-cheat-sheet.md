...
Quick reference for the most commonly used Confluence macros. Each entry includes the macro name,
storage format syntax, primary use case, and practical tips.
...
- **Parameters:** `maxLevel` (1-6), `minLevel` (1-6), `style` (disc, circle, square, none), `type`
  (list, flat)
...
- **Parameters:** `depth` (1-999), `sort` (title, creation, modified), `style` (h2-h6), `all`
  (true/false)
...
- **Syntax:**
  `<ac:structured-macro ac:name="note"><ac:rich-text-body>Content</ac:rich-text-body></ac:structured-macro>`
...
- **Syntax:**
  `<ac:structured-macro ac:name="tip"><ac:rich-text-body>Content</ac:rich-text-body></ac:structured-macro>`
...
- **Parameters:** `jqlQuery`, `columns` (key, summary, status, assignee, etc.), `count`
  (true/false), `serverId`
